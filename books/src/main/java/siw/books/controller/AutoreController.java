...
public class AutoreController {
	...
	public String saveAutore(@RequestParam("nome") String nome, @RequestParam("cognome") String cognome,
	...
	public String modificaAutore(@PathVariable Long id, @ModelAttribute Autore autoreModificato,
			@RequestParam(name = "libriIds", required = false) List<Long> libroIds,
			@RequestParam("foto") MultipartFile nuovaFoto, Model model) throws IOException {
		...
		if (opt.isPresent()) {
			...
			if (nuovaFoto != null && !nuovaFoto.isEmpty()) {
				try {
					...
					String nomeFile = (foto != null) ? foto.getNomeFile() : "default.jpg"; // oppure null, o "" se vuoi
																							// saltarlo
	...
	public String ordinaAutoriAnno(Model model, Authentication authentication) {
		...
		autoriTrovati.sort((a1, a2) -> {
			if (a1.getDataNascita() == null && a2.getDataNascita() == null)
				return 0;
			if (a1.getDataNascita() == null)
				return 1;
			if (a2.getDataNascita() == null)
				return -1;
